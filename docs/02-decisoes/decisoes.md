...
> Formato curto: contexto → decisão → consequência. Decisões da v2.3 mantidas + novas da revisão v3.0 (junho 2026) + consolidação de jun/2026 (ADR-020/021).
...
**Contexto (20/06/2026):** site mira nível Awwwards, mas com metas de Core Web Vitals e orçamento de tempo. Cliente escolheu "decidir por seção". **Decisão:** esforço pesado no hero, na jornada fase a fase e no bloco Herança; médio no scroll horizontal; leve em culturas, números, experience, depoimentos e footer. **Consequência:** investimento concentrado onde o retorno visual é maior; resto sóbrio e barato. Detalhe em `docs/01-prd/PRD.md` (seção 7).

## ADR-019 — Jornada da home: seção fixa (pin) com vídeo em auto-play, travado até terminar
**Contexto (22/06/2026):** a mecânica foi sendo refinada ao ver funcionando. Versão final decidida pelo cliente: a primeira seção é **uma só** (hero + jornada), montada em camadas (vídeo atrás → headline → campo → folhas na frente). **Decisão:** ao rolar, a seção **prende (trava o scroll)** e o **vídeo toca sozinho uma vez** (auto-play, ritmo normal), com os textos das fases trocando; o scroll fica **travado até a animação terminar**, aí solta. **NÃO é scrub** (o usuário não controla o vídeo quadro a quadro). Os textos seguem a estética do hero: Montserrat Black, à esquerda, no espaço branco, nunca sobre a imagem. **Consequências:** GSAP ScrollTrigger com pin; `play()` no enter, solta no `ended`; crossfade imagem→vídeo (a imagem é o 1º frame); mobile leve; `prefers-reduced-motion` estático. Substitui as versões anteriores (trigger-based scroll-livre do ADR-016 e o scrub que esta ADR descrevia antes).

## ADR-018 — Painel admin adota a linguagem visual do Media Hub by Ocean
**Contexto (20/06/2026):** o cliente (Oceon) quer que o painel da Juma use a estética do Media Hub, produto da própria agência: workspace claro arejado + sidebar escura flutuante, glassmorphism, soft shadows, Geist, tokens de status (âmbar/verde/vermelho/azul). **Decisão:** adotar essa linguagem como base do painel, aplicada via tema e componentes customizados do Payload (custom.scss com as variáveis, Nav custom para a sidebar flutuante com a estrutura multi-projeto, logo, view de Dashboard, login). **Consequência:** o painel ganha a cara da Oceon nos pontos de personalidade (sidebar, dashboard, login, branding); as telas internas de CRUD ficam restilizadas, não recriadas (estrutura de tabela/form do Payload permanece). Estimativa ~85% da linguagem. Trabalho de Fase 5; fundação do tema pode vir antes. Tokens e plano em `docs/01-prd/painel-admin.md`.
...
## ADR-020 — PRD consolidado em fonte única; painel + 3 idiomas reafirmados (reverte o escopo do "PRD v4")
**Contexto (24/06/2026):** a documentação tinha três camadas que se contradiziam — PRD v3.0 (`docs/01-prd-site/`), direção de design (`docs/05-design-direction/`) e um "PRD v4" (`docs/06-prd-v4/`) que, para ser autossuficiente como prompt de IA, declarou **fora de escopo** o painel, o CMS, o banco e os idiomas, propondo um site só em pt-BR com dados estáticos. O código real, porém, já implementava o caminho do v3: next-intl com pt-BR/en/es, Payload com localização nos três idiomas e todas as coleções. **Decisão (cliente):** o projeto segue **com painel administrativo e com os três idiomas**. Consolidar tudo em uma fonte de verdade única — `docs/01-prd/` — incorporando a melhor pesquisa de stack/efeitos do v4 (vídeo por `pin`, motor de animação único, tipografia em três vozes) **sobre** o escopo completo (painel + i18n + CMS). Aposentar o PRD v3.0 e a pasta de design (conteúdo dobrado no canônico ou movido para `docs/01-prd/`). **Consequência:** uma só fonte de verdade, sem contradição entre doc e código; o framing "só pt-BR / sem painel" do v4 fica revertido e registrado aqui.

## ADR-021 — Motor de animação único (GSAP + ScrollTrigger + Lenis); R3F e Framer Motion fora da stack
**Contexto (24/06/2026):** o `CLAUDE.md` listava Three.js, React Three Fiber e Framer Motion como stack de animação "instalada", mas o `package.json` tem apenas `gsap`, `@gsap/react` e `lenis`. A pesquisa do PRD (seção 2.2) concluiu que somar uma segunda biblioteca de animação de UI é redundância sem ganho. **Decisão:** o motor de animação é **um só — GSAP + ScrollTrigger + Lenis**. **Three.js/WebGL** entra **só sob demanda** para o momento de maior impacto da jornada (efeito de água), via import dinâmico, **ainda não instalado**, e condicionado ao gate de validação de performance (ADR-011). **React Three Fiber e Framer Motion ficam fora da stack.** As skills de animação instaladas (`react-three-fiber`, `motion-framer`, `threejs-webgl`, etc.) são **referência**, não dependências do projeto. **Consequência:** menos peso e manutenção; o `CLAUDE.md` passa a descrever a stack realmente instalada.

## ADR-016 — Topo da home como filme contínuo (jornada + herança + morph → produtos)
**Contexto (20/06/2026):** o cliente propôs costurar a jornada agronômica e a herança num único scroll contínuo, usando a gota como ponte para a origem da empresa, terminando numa transição em vídeo do frasco antigo para o atual que desemboca no scroll horizontal de produtos. **Decisão:** adotado. O topo da home (hero, jornada, herança, morph) vira uma sequência contínua de 8 quadros; o bloco Herança deixa de ser separado e passa a viver dentro da jornada; o argumento racional (como funciona, números) reflui para depois do scroll de produtos. **Consequências:** (1) o morph dos frascos usa frames de vídeo (baixo risco), e só a gota com fundo ondulado fica no gate do ADR-011; (2) montar os textos da herança sobre fundo ondulado empurra o efeito de água para o plano B mais seguro; (3) sequência longa exige versão mobile mais curta, skip via nav/CTA e `prefers-reduced-motion` estático; (4) todo texto fica no DOM por SEO/acessibilidade. Detalhe e copy em `docs/01-prd/PRD.md` (seção 7) e `docs/04-copy/01-home.md`.
