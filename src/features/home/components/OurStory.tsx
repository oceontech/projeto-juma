...
export function OurStory() {
  ...
  useGSAP(
    () => {
      ...
      const stats     = statsRootRef.current ? gsap.utils.toArray<HTMLElement>('[data-stat]', statsRootRef.current) : []
      const labels    = isDesktop && labelsRootRef.current ? gsap.utils.toArray<HTMLElement>('[data-label]', labelsRootRef.current) : []
      const vLines    = isDesktop && labelsRootRef.current ? gsap.utils.toArray<HTMLElement>('[data-vline]', labelsRootRef.current) : []
      const hLines    = isDesktop && labelsRootRef.current ? gsap.utils.toArray<HTMLElement>('[data-hline]', labelsRootRef.current) : []
      ...
      const trigger = ScrollTrigger.create({
        ...
        onEnter:     () => { exit.pause(0); entry.restart() },
        onEnterBack: () => { exit.pause(0); entry.restart() },
        onLeave:     () => { entry.pause(0); exit.restart() },
    ...
    <div ref={sectionRef} className="w-full relative z-10 py-10 md:py-20" style={{ perspective: '1000px' }}>
      ...
      <section ref={cardRef} className="relative overflow-hidden rounded-[2.5rem] max-w-[95vw] lg:max-w-[90rem] mx-auto border border-black/[0.06] bg-gradient-to-br from-black/[0.01] to-black/[0.04] backdrop-blur-xl" style={{ transformOrigin: 'top center' }}>
        ...
        <div className="flex flex-col items-start">
          ...
          <div ref={ctaRef} className="mt-lg lg:mt-xl inline-flex items-center gap-sm rounded-full border border-primary/30 px-lg py-sm">
...
function FounderLabel({ align, x, lineHeight, name, role }: {
  align: FounderAlign; x: number; lineHeight: number; name: string; role: string
}) {
